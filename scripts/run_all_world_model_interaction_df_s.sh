...
model_name=${MODEL_NAME:-${1:-single_arm_franka_simulation/epoch=75-step=5000}}
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-${2:-7}}
...
bash "${SCRIPT_DIR}/run_world_model_interaction_df_s.sh" "${model_name}" "${CUDA_VISIBLE_DEVICES}"
if [ $? -ne 0 ]; then
...
bash "${SCRIPT_DIR}/run_world_model_interaction_df_s_multiview.sh" "${model_name}" "${CUDA_VISIBLE_DEVICES}"
if [ $? -ne 0 ]; then
...
bash "${SCRIPT_DIR}/run_world_model_interaction_df_s_unseen.sh" "${model_name}" "${CUDA_VISIBLE_DEVICES}"
if [ $? -ne 0 ]; then
...
bash "${SCRIPT_DIR}/run_world_model_interaction_df_s_OOD.sh" "${model_name}" "${CUDA_VISIBLE_DEVICES}"
if [ $? -ne 0 ]; then
